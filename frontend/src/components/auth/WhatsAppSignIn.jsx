...
const WhatsAppSignIn = ({ onSwitchToSignUp, onShowPopup }) => {
  ...
  const handleVerifyOTP = async (otp) => {
    ...
    try {
      ...
      if (!result.success) {
        // Check if it's a "signup first" error
        if (result.error && result.error.includes('Signup First')) {
          console.log('🎯 Showing Signup Required popup');
          onShowPopup({
            isOpen: true,
            type: 'warning',
            title: 'Signup Required',
          ...
          // Switch to signup mode after popup closes
          setTimeout(() => {
...
const OTPVerification = ({ phoneNumber, onVerify, isLoading, onBack }) => {
